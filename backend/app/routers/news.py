...
def get_news(skip: int = 0, limit: int = 20, db: Session = Depends(get_db)):
    return db.query(News).options(joinedload(News.author)).order_by(News.created_at.desc()).offset(skip).limit(limit).all()
...
def create_news(body: NewsCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    news = News(title=body.title, body=body.body, video_url=body.video_url, author_id=user.id)
    db.add(news)
    db.commit()
    db.refresh(news)
    return news
